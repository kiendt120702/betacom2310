...
// @ts-ignore
const geminiApiKey = Deno.env.get('GEMINI_API_KEY'); // New API key for Gemini
// @ts-ignore
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Processing SEO Product Title chat message:', message);

    // Step 1: Get conversation history if conversationId exists
    let conversationHistory: any[] = [];
    if (conversationId) {
      const { data: historyData, error: historyError } = await supabase
        .from('seo_product_title_chat_messages') // New table for this bot
        .select('role, content')
    ...
    // Step 2: Generate embedding for user query using OpenAI
    ...
    // Step 6: Create system prompt for SEO Product Title consultant
    const systemPrompt = `Bạn là chuyên gia SEO Shopee chuyên nghiệp, CHỈ tập trung vào việc tư vấn và tạo TÊN SẢN PHẨM chuẩn SEO.

NGUYÊN TẮC HOẠT ĐỘNG:
1. Phân tích yêu cầu người dùng chính xác dựa trên ngữ cảnh cuộc hội thoại.
2. CHỈ làm đúng những gì được yêu cầu, đặc biệt là CHỈ TRẢ VỀ TÊN SẢN PHẨM.
...
B. KHI NGƯỜI DÙNG HỎI VỀ LÝ THUYẾT/QUY TẮC/CÁCH THỨC ĐẶT TÊN SẢN PHẨM SEO:
- Thêm câu mở đầu: "Để giúp bạn hiểu rõ hơn về cách đặt tên sản phẩm chuẩn SEO Shopee, tôi sẽ giải thích:"
- Giải thích chi tiết quy tắc đặt tên sản phẩm.
- Hướng dẫn từng bước.
- Đưa ra ví dụ minh họa.
- Kết thúc: "Bạn có câu hỏi nào khác về cách đặt tên sản phẩm không?"
...
D. KHI NGƯỜI DÙNG NHẮC ĐẾN "SẢN PHẨM NÀY/ĐÂY" MÀ KHÔNG CÓ THÔNG TIN CỤ THỂ:
...
Hãy phân tích yêu cầu của người dùng dựa trên ngữ cảnh cuộc hội thoại và trả lời chính xác theo nguyên tắc trên.`;
    ...
    if (!geminiResponse.ok) {
      ...
      throw new Error(`Failed to generate AI response from Gemini: ${geminiResponse.status}`);
    }

    const geminiData = await geminiResponse.json();
    const aiResponse = geminiData.candidates?.[0]?.content?.parts?.[0]?.text || 'Xin lỗi, tôi không thể tạo phản hồi lúc này.';
    const cleanedAiResponse = stripMarkdown(aiResponse);
    ...
    if (conversationId) {
      ...
      // Save user message
      await supabase.from('seo_product_title_chat_messages').insert({
    ...
    console.log('SEO Product Title chat response generated successfully');
    ...
    console.error('Error in seo-product-title-chat function:', error);
    return new Response(JSON.stringify({ 
      error: error.message,
