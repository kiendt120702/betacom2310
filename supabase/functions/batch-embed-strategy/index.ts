...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Starting batch embedding for strategy knowledge...');
    ...
    if (!documents || documents.length === 0) {
      return new Response(JSON.stringify({ 
        message: 'Không có tài liệu nào cần embedding',
        processed: 0
      }), {
    ...
    for (const doc of documents) {
      try {
        const content = `${doc.formula_a1} ${doc.formula_a} ${doc.industry_application}`;
    ...
    console.error('Error in batch-embed-strategy function:', error);
    return new Response(JSON.stringify({ 
      error: 'Đã có lỗi xảy ra khi batch embedding strategy knowledge' 
    }), {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
