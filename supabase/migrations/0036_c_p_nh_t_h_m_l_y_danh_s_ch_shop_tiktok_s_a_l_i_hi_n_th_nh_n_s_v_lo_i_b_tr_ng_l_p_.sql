...
latest_shop_entries AS (
  -- Get the single latest entry for each shop name to represent it
  SELECT DISTINCT ON (name) *
  FROM public.tiktok_shops
  ORDER BY name, created_at DESC
)
...
FROM (
    ...
    FROM latest_shop_entries s
    LEFT JOIN shop_assignments sa ON s.name = s.name
