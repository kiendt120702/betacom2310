...
WITH shop_assignments AS (
  -- Tìm một bản ghi có phân công nhân sự cho mỗi tên shop
...
FROM (
    SELECT
        ...
        (
            ...
            -- Sử dụng profile_id đã có, hoặc lấy từ bản ghi đã được phân công
            WHERE p.id = COALESCE(s.profile_id, sa.profile_id)
        ) as profile
