...
import { useUpdateGeneralTrainingVideo, useDeleteGeneralTraining } from '@/hooks/useGeneralTraining';
import { GeneralTrainingExercise } from '@/hooks/useGeneralTraining';

interface GeneralVideoDeleteDialogProps {
  exercise: GeneralTrainingExercise;
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSuccess?: () => void;
  deleteVideoOnly?: boolean; // True = chỉ xóa video, False = xóa cả bài học
}

const GeneralVideoDeleteDialog: React.FC<GeneralVideoDeleteDialogProps> = ({
  exercise,
  open,
  onOpenChange,
  onSuccess,
  ...
  const updateGeneralTrainingVideo = useUpdateGeneralTrainingVideo();
  const deleteGeneralTraining = useDeleteGeneralTraining();
  ...
  const handleDelete = async () => {
    ...
    try {
      if (deleteVideoOnly) {
        ...
        await updateGeneralTrainingVideo.mutateAsync({
          exerciseId: exercise.id,
          videoUrl: null,
        });
      } else {
        // Delete entire exercise (this will also delete video from storage)
        await deleteGeneralTraining.mutateAsync(exercise.id);
      }
...
export default GeneralVideoDeleteDialog;