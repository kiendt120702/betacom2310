...
const UserTable: React.FC<UserTableProps> = ({ users, currentUser, onRefresh }) => {
  ...
  const canEditUser = (user: UserProfile) => {
    ...
    if (isLeader) { // Leaders can edit 'chuyên viên'
      return user.role === 'chuyên viên';
    }
    return false;
  };
  ...
  const canDeleteUser = (user: UserProfile) => {
    if (isAdmin) return true;
    if (isLeader) { // Leaders can delete 'chuyên viên'
      return user.role === 'chuyên viên' && user.id !== currentUser?.id;
    }
    return false;
  };
