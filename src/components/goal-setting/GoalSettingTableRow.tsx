...
import { ShopReportData } from '@/types/reports';
import { useShops } from '@/hooks/useShops';

interface GoalSettingTableRowProps {
  shop: ShopReportData;
  index: number;
}

const GoalSettingTableRow: React.FC<GoalSettingTableRowProps> = ({ shop, index }) => {
  const { updateReportMutation, filters, setEditingShop, setIsShopDialogOpen } = useGoalSettingContext();
  const { data: currentUserProfile } = useUserProfile();
  const { isAdmin, isLeader, isChuyenVien } = useUserPermissions(currentUserProfile);
  
  const { data: allShopsData } = useShops({ page: 1, pageSize: 10000, searchTerm: "", status: "all" });
  ...
  const handleEditShop = () => {
    const shopToEdit = allShops.find(s => s.id === shop.shop_id);
    if (shopToEdit) {
    ...
    <TableRow>
      ...
      <TableCell className="whitespace-nowrap text-right">
        {isEditing ? (
          <Input
            ...
            onChange={(e) => handleGoalInputChange('feasible_goal', e.target.value)}
      ...
      <TableCell className="whitespace-nowrap text-right">
        {isEditing ? (
          <Input
            ...
            onChange={(e) => handleGoalInputChange('breakthrough_goal', e.target.value)}
            className="w-28 text-right h-8 px-2 py-1"
            disabled={updateReportMutation.isPending}
            placeholder="0"
          />
      ...
      <TableCell className="text-right">
        <div className="flex items-center justify-end gap-2">
          {isEditing ? (
            <>
              <Button variant="outline" size="sm" onClick={handleCancel} disabled={updateReportMutation.isPending}>Hủy</Button>
              <Button size="sm" onClick={handleSave} disabled={updateReportMutation.isPending}>
                {updateReportMutation.isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Lưu...</> : "Lưu"}
              </Button>
            </>
          ) : (
            <>
              <Button variant="ghost" size="icon" onClick={handleEditShop} className="h-8 w-8 p-0" title="Sửa thông tin shop" disabled={!canEditShopDetails}>
