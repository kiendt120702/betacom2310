...
const ApprovalDialog = ({ banner, open, onOpenChange }: ApprovalDialogProps) => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'pending':
        return <Badge variant="outline" className="bg-yellow-100 text-yellow-800">Chờ duyệt</Badge>;
      case 'approved':
        return <Badge variant="outline" className="bg-green-100 text-green-800">Đã duyệt</Badge>;
      case 'rejected':
        return <Badge variant="outline" className="bg-red-100 text-red-800">Đã từ chối</Badge>;
      default:
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <div className="text-sm text-muted-foreground">
            Được tạo bởi: {banner.user_name || 'Không xác định'}
          </div>
        </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row justify-end gap-2 pt-4 border-t">
              ...
              <Button
                ...
                disabled={approveBannerMutation.isPending}
              >
                {approveBannerMutation.isPending ? 'Đang duyệt...' : 'Duyệt ngay'}
              </Button>
              ...
              <Button
                ...
                disabled={approveBannerMutation.isPending}
              >
                {approveBannerMutation.isPending ? 'Đang cập nhật...' : 'Cập nhật & Duyệt'}
              </Button>
            </div>
          </div>
        </Form>
