...
export const useComprehensiveReportData = ({
  ...
  const { data: userProfile } = useUserProfile(); // Get current user's profile
  ...
  const processedData = useMemo((): ShopReportData[] => {
    ...
    if (userProfile?.role === "chuyên viên") {
      filtered = filtered.filter(
        (shop) => shop.personnel_id === userProfile.id,
      );
    }
    ...
    if (filters.selectedPersonnel !== "all") {
      filtered = filtered.filter(
        (shop) => shop.personnel_id === filters.selectedPersonnel,
      );
    } else if (filters.selectedLeader !== "all" && allShops) {
      // If no specific personnel, filter by leader
      const leaderShops = allShops.filter(
        (shop) => shop.profile?.manager?.id === filters.selectedLeader,
      );
    ...
    if (sortConfig) {
      ...
      filtered.sort((a, b) => {
        const dateA = a.last_report_date
          ? new Date(a.last_report_date).getTime()
          : 0;
        const dateB = b.last_report_date
