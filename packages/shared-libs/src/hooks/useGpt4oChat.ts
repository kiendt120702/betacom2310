...
export const useConversations = () => {
  ...
  return useQuery<Conversation[]>({
    queryKey: ["gpt4o-conversations", user?.id],
    queryFn: async () => {
      if (!user) return [];
      try {
        const { data, error } = await supabase
          .from("gpt4o_mini_conversations")
          .select("*")
          .eq("user_id", user.id)
...
export const useMessages = (conversationId: string | null) => {
  return useQuery<Message[]>({
    queryKey: ["gpt4o-messages", conversationId],
    queryFn: async () => {
      if (!conversationId) return [];
      try {
        const { data, error } = await supabase
          .from("gpt4o_mini_messages")
          .select("*")
...
export const useCreateConversation = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["gpt4o-conversations", user?.id] });
    },
    onError: (error: any) => {
...
export const useAddMessage = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["gpt4o-messages", data.conversation_id] });
    },
    onError: (error: any) => {
